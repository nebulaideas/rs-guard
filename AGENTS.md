...
**Current Status:** Phases 1–7 complete; v1.8.0 released. Multi-pass review, .rs-guardignore, language-aware prompts, llm-kernel hybrid architecture, per-variant temperature override. Released via crates.io only (`cargo install rs-guard --locked`); pre-built binaries deferred until llm-kernel switches from aws-lc-rs to ring (upstream issue #93).
...
│   │   ├── generic_client.rs      # GenericOpenAiCompatibleClient (deepseek, qwen, kimi)
│   │   ├── kernel_client.rs       # KernelBackedClient (6 standard providers, wraps llm-kernel)
