...
| `llm_timeout_secs`  | integer | `120` (240 for deepseek/kimi) | Total timeout (seconds) for LLM `/chat/completions` HTTP requests. Auto-raised to 240s for `deepseek` and `kimi` when not explicitly set (to support reasoning models like deepseek-v4-pro). Increase further for complex PRs. |
